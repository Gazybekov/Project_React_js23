...
const CartContextProvider = ({ children }) => {
  ...
  //! GET
  // функция для получения продуктов добавленных в коризну их хранилища
  const getCart = () => {
  ...
  const addProductToCart = (product) => {
    // получаем содержимое их хранилища под ключом cart
    let cart = getLocalStorage();
    if (!cart) {
      cart = {
        products: [],
    ...
    // проверяем есть ли уже продукт который хотим добавить в коризну
  ...
  const changeProductCount = (id, value) => {
    let cart = getLocalStorage();
    // перебираем массив с продуктами из корзины, и у продукта, у которого id совпадает с элементом, что передали при вызове, перезаписываем count (кол-во) и subPrice
